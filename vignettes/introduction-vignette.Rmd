...
We note here that this function only accepts one `how` per run. To analyze the results of multiple `build` parameter searches jointly, combine the results of those `plGrid` function calls using `?conjoin`. We will also note that `plGrid` does not execute any data splitting or feature selection, both of which the user may perform beforehand. However, `plGrid` does allow the user to specify multiple *classifier sizes* by providing a numeric vector as the `top` argument.
...
```{r}
pipeSubset(gs, colBy = "kernel", include = "linear")
subset(gs, subset = gs$kernel == "linear")
```
