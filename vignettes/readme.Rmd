...
* **pipeFilter, buildEnsemble:** For an `ExprsPipeline` model extraction, if you supply an $x$ number of top models to the `top` argument greater than the total number of models available in a filtered cut of models, *exprso* will automatically use *all* models instead. If you are concerned about this, call `pipeFilter` first, then call `buildEnsemble` on the `pipeFilter` results after inspecting them manually.
