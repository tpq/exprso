...
pl <- plGridMulti(trainingSet(splitSets), testSet(splitSets), ctrlFS = fs, top = c(2, 3, 0),
...
```{r, results = "hide"}
fs <- ctrlFeatureSelect(func = "fsStats", top = 0, how = "t.test")
gs <- ctrlGridSearch(func = "plGridMulti", ctrlFS = fs, top = 3,
                     how = "buildSVM", kernel = c("linear", "radial"), gamma = c(.1, .2))
fs.outer <- ctrlFeatureSelect(func = "fsNULL", top = 0)

pl <- plNested(trainingSet(splitSets), fold = 10,
               ctrlFS = fs.outer, ctrlGS = gs)
```
