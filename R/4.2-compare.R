...
#'  variable).
#' @param include A vector. The terms found in \code{colBy} to include when
#'  comparing all other annotation terms.
...
setMethod("compare", "ExprsArray",
          function(object, array.valid = NULL,
                   colBy = "defineCase",
                   include = unique(object@annot[, colBy]),
                   cutoff = .05){
            ...
            # Subset all provided arrays
            object <- object[object@annot[, colBy] %in% include, ]
            if(!is.null(array.valid)){
              ...
              array.valid <- array.valid[array.valid@annot[, colBy] %in% include, ]
            }
            ...
            for(i in 1:length(results)){
              ...
              if(!class(test.data[, colBy]) %in% c("character", "factor")){
                ...
                warning("Expect strange results when 'colBy' does not contain categorical data!\n")
              }
